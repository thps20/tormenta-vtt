...
describe("suggestDamage", () => {
  it("alvo sem resposta a dano: ×1 e aviso vazio", () => {
    ...
    expect(result).toEqual({ multiplier: "1", amount: 10, note: "" });
  });
  ...
  it("vulnerável: ×2", () => {
    ...
    const result = suggestDamage(def, [dmg("2d6", 10)], responses);
    expect(result).toEqual({ multiplier: "2", amount: 20, note: "Vulnerável a fogo" });
  });
  ...
  it('"reduz à metade": ×½ (arredondado pra baixo)', () => {
    ...
    expect(result).toEqual({ multiplier: "0.5", amount: 4, note: "Resistente a fogo" });
  });
  ...
  it("RD: sem multiplicador (null), valor já com a subtração (piso 0)", () => {
    ...
    expect(suggestDamage(def, [dmg("2d6", 10)], responses)).toEqual({ multiplier: null, amount: 5, note: "Resistente a fogo (RD 5)" });
    expect(suggestDamage(def, [dmg("2d6", 3)], responses)).toEqual({ multiplier: null, amount: 0, note: "Resistente a fogo (RD 5)" });
  });
  ...
  it("all (resposta geral) entra em toda parcela, mesmo sem tipo", () => {
    ...
    expect(suggestDamage(def, [dmg("1d6", 6, null)], responses)).toEqual({ multiplier: null, amount: 4, note: "Resistente (RD 2)" });
  });
  ...
  it("cura nunca é ajustada, mesmo com resposta a dano cadastrada", () => {
    ...
    expect(result).toEqual({ multiplier: "1", amount: 12, note: "" });
  });
});
