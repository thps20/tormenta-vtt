...
export const MapConfigModal: React.FC<MapConfigModalProps> = ({ isOpen, scene, systemDef, onSave, onClose }) => {
  ...
  // Esc fecha. O listener NÃO pode depender de `onClose` (uma arrow nova a cada render do pai):
  // o próprio Esc também cancela o gesto da ferramenta, o que re-renderiza a página no meio do
  // evento — o efeito então removia este listener e registrava outro, e um listener adicionado
  // durante o disparo não é chamado. Com o ref, registra uma vez só enquanto o modal está aberto.
