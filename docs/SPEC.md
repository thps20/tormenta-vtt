...
- **Barra de ferramentas** (coluna à esquerda do canvas, um modo por vez, estado em `store/tools.ts`, atalhos em `lib/useToolShortcuts.ts`):
  ...
  - **Régua (R)**: clicar e arrastar mede do ponto inicial ao ponteiro (pontos grudam no centro da célula quando há grid e snap), igual sempre; soltar sem Ctrl com nenhum vértice travado mede e some, sem mudar nada do comportamento de antes. Soltar (ou clicar) com **Ctrl** trava um vértice ali e continua medindo dali pra frente — sem precisar manter o botão pressionado entre um vértice e outro — construindo um caminho com vários trechos; Backspace remove o último vértice travado (com só o trecho "cru" ainda sem nenhum vértice, apaga a régua inteira); Esc limpa a régua inteira a qualquer momento. Um caminho com vértices, ao ser finalizado (soltar sem Ctrl), CONTINUA visível/sincronizado em vez de sumir — só reflete a decisão de investir num caminho com curvas. A distância usa `grid` do `SystemDefinition` sobrescrito pela escala do MAPA quando `Scene.grid.unitsPerCell`/`unit` estão definidos (`withMapScale`; `cellSize` na unidade do jogo, `unit`, regra de diagonais `euclidean | manhattan | alternating | chebyshev` sempre do sistema; `rules/measure.ts#measurePath` faz a conta, acumulando diagonais ENTRE trechos como a regra 1-2-1 do d20 exige) e o `cellSize` em px da cena; mostra a distância de cada trecho (com mais de um) e o total. Com um combate ativo e o token da vez ENTRE os selecionados, mostra também "gasto / orçamento" (`7,5 / 9 m`) medindo o caminho da régua a partir do gasto já feito no turno, com as MESMAS funções puras do orçamento de deslocamento (§9.11, `rules/movement.ts`) — sem gastar orçamento de verdade, só leitura. A régua é enviada por `ruler:update` (efêmero, `ruler.points`) e os outros a veem com o nickname do autor; some ao soltar (exceto um caminho com vértices, que fica até Esc ou trocar de ferramenta).
...
- **Dados rápidos** (botões `d4..d100` na faixa "Rolar" do chat): clicar **acumula** na fórmula do campo (`/r <fórmula>`), nunca rola na hora — clicar `d6` duas vezes vira `2d6`, preservando modificador e `# rótulo` já digitados (`addDieToFormula`, `packages/shared/src/dice/parser.ts`). Só Enter/o botão enviar rola de verdade.
...
- **Ícone do dado no cartão**: um ícone por tipo de dado presente na fórmula (SVG monocromático embutido, `apps/web/src/components/icons/dice/DiceIcon.tsx`), com a contagem ("3× d6"); fórmulas mistas mostram um ícone por tipo, agrupando grupos repetidos do mesmo lado (`summarizeDiceTypes`, `packages/shared/src/dice`). O ícone do d20 fica dourado em crítico e vermelho em falha crítica; os demais tipos ficam neutros.
- **Animação de rolagem** (preferência por usuário, `localStorage`, padrão "simples", botão de 3 estados na faixa "Rolar"): "simples" toca uma transição curta (~400ms) no próprio cartão antes de assentar no resultado; "3D" mostra dados físicos caindo sobre o mapa (`DiceOverlay3D.tsx`, `@3d-dice/dice-box-threejs` carregado sob demanda só quando ligado — nunca decide o resultado, só encena o que o servidor já mandou; falha ao carregar/inicializar cai pro modo "simples" pelo resto da sessão, com um aviso único); "desligada" não anima nada. Em qualquer modo: clicar no resultado revela na hora, uma rolagem em lote (iniciativa) anima uma vez só, a animação nunca atrasa o registro da mensagem no chat, e `prefers-reduced-motion` pula direto pro resultado.
...
| `compendium:room-create` / `compendium:room-update` | `{ entry: RoomCompendiumEntryInput }` / `{ entryId, entry }` — a entrada mecânica INTEIRA, sem id (create) ou pelo id existente (update) | GM | ack `CompendiumEntry`; `compendium:room-created`/`-updated` (§9.18) |
| `compendium:room-delete` | `{ entryId }` | GM | ack; `compendium:room-deleted` (soft delete, entra no desfazer, §9.18) |
| `compendium:room-export` | `{}` | GM | ack `{ entries: CompendiumEntry[] }` do que não está apagado; sem broadcast (§9.18) |
| `compendium:room-import` | `{ entries: CompendiumEntry[], overwriteConflicts }` | GM | ack `{ imported, overwritten, skipped: [{id,name,reason}] }`; `compendium:room-created`/`-updated` por entrada alterada (§9.18) |
...
| `ruler:update` | `{ sceneId, ruler: { points: [{x,y}, ...] } \| null }` (2+ pontos, pixels do mapa; Ctrl trava vértices no meio, §3.2) | todos | `ruler:updated` para os **outros** (efêmero: não persiste; `null` apaga) |
...
| `compendium:room-created` / `compendium:room-updated` | `CompendiumEntry` (§9.18; só pra `rooms.gm`, cliente faz upsert por id em `entries`/`roomIds`) |
...
- Ficha: o compêndio (§9.4) vem dos packs do Foundry e tem lacunas listadas em `scripts/import-report.md` (proficiências e limite de atributo das armaduras pesadas, sentidos/perícias das raças, páginas ausentes, fórmulas com variáveis do Foundry). Sentidos da raça não alimentam `derived[]` (§9.11: deslocamento já alimenta, via `{race.movement}`); poderes de classe por nível ficam de fora. Consumíveis usam a mesma ativação de poderes/magias, mas a quantidade não é descontada ao usar.
...
- **Servidor**: `compendium:list` devolve `mergeCompendium([sistema, sala])`; a fonte da sala (homebrew do GM, prioridade maior, id repetido substitui) lê a tabela `RoomCompendiumEntry` — ver §9.18.
...
### 9.18 Homebrew da sala

O GM cria e edita conteúdo próprio (armas, armaduras, magias, poderes, itens, criaturas) direto no
compêndio da SALA, que já tinha prioridade sobre o do sistema quando o id coincide (`mergeCompendium`,
§9.4) mas era um stub vazio até aqui (setembro/2026). Plano e decisões em
`docs/plano-compendio-sala.md`.

- **Modelo**: `RoomCompendiumEntry` (`apps/server/prisma/schema.prisma`) — chave primária **composta**
  `(roomId, entryId)`: `entryId` é o mesmo id legível (slug do nome) que vira `CompendiumEntry.id` e
  decide a prioridade em `mergeCompendium`; só precisa ser único DENTRO da sala, então um cuid a mais
  não serviria pra nada. Colunas `type` ("item"/"creature"), `kind` (chave de `itemKinds[]`, só em
  item), `name`, `tags[]`, `description`, `page` (índice/exibição sem precisar ler `data`); `data`
  (Json) guarda o resto do corpo mecânico — `CompendiumItemBody` (menos `description`/`page`, já
  colunas) ou `{ sheet: CreatureSheet }`. `entryId` é gerado do nome só na **criação**
  (`slugify`, `packages/shared/src/rules/compendium.ts`) e nunca muda depois (sem campo pra editar —
  decisão confirmada, evita sobrepor uma entrada do sistema sem querer); colisão de slug — inclusive
  com uma linha já apagada da PRÓPRIA sala (a chave primária não distingue), ou com um id do
  compêndio do SISTEMA — ganha sufixo numérico (`nextEntryId`), mesma regra do importador offline.
  `roomCompendiumSource` (`apps/server/src/services/compendium.ts`) lê a tabela pra `compendium:list`;
  uma linha inválida (não deveria acontecer, já validada na gravação) fica de fora com aviso no log,
  não derruba a sala.
- **Eventos** (`compendium:room-*`, gmOnly, broadcast pra `rooms.gm` — mesmo desenho de `handout:*`):
  `create`/`update` (`RoomCompendiumEntryInputSchema`, a MESMA entrada mecânica inteira que
  `CompendiumItemEntry`/`CompendiumCreatureEntry` sem `id` — nunca um patch parcial),
  `delete` (soft delete, `deletedAt`), `export` (tudo que não está apagado) e `import` (lote de
  `CompendiumEntry[]` completos, tipicamente vindos de um `export` de outra sala). `import` valida
  CADA entrada sozinha — uma ruim é pulada e reportada, não derruba o lote; id que já existe ATIVO na
  sala é pulado por padrão (checkbox "sobrescrever conflitos" desmarcado na tela) ou sobrescrito se
  marcado; id que só existe APAGADO é revivido e contado como importado (do ponto de vista do GM não
  havia nada ali). Ack de `import`: `{ imported, overwritten, skipped: [{ id, name, reason }] }`.
  **Apagar entra no desfazer do GM** (Ctrl+Z): `revert` volta `deletedAt: null`, mesmo padrão de
  `handout:delete` — diferente de `encounter:delete`, que não tem undo.
- **Editor: reaproveita a FICHA, não um formulário novo** (`RoomEntryEditor`,
  `apps/web/src/components/compendium/`): monta uma "ficha de personagem" em memória, nunca
  persistida (`kind: "npc"`, id fixo `compendium-room-editor-draft`), e renderiza as MESMAS seções da
  ficha contra ela — `onPatch` só atualiza esse rascunho local, sem emitir socket. Item: só
  `ItemsSection`, com uma cópia do `SystemDefinition` cujo `itemKinds` tem UM item só (o tipo
  escolhido), então a seção nem mostra abas de outro tipo nem deixa um segundo item sobreviver (se o
  GM clicar "+" de novo, só o mais recente fica — `onPatch` corta a lista pra 1). Criatura: a ficha
  inteira (atributos, recursos, derivados, perícias, itens de qualquer tipo, modificadores), sem
  cabeçalho de participante nem "Anotações" (bio/imagem não existem em `CreatureSheet` — descrição e
  página ficam em dois campos próprios no editor, junto com o nome e as tags). Salvar converte o
  rascunho pro formato do compêndio com `characterItemToCompendiumBody`/`creatureSheetFromCharacterData`
  (`rules/compendium.ts`, o caminho INVERSO de `entryToItem`/`entryToCharacter`) e roda
  `validateCompendiumEntry` no cliente antes de emitir, pra mostrar o erro sem gastar uma emissão de
...
- **UI, tudo dentro da paleta que já existe** (nada de tela separada): botão **"Novo ▾"** no
  ...
  para a sala"** no preview (`RoomEntryActionsRow`, compartilhado entre `EntryPreview`/
  `CreaturePreview`); uma entrada já da SALA (identificada por `roomIds`, que já existia) troca esse
  botão por **"Editar"**/**"Apagar"**. Botão **"Sala…"** no cabeçalho abre exportar/importar
  (`RoomCompendiumImportExport`): exportar baixa um `.json` com as entradas da sala (Blob, sem rota
  HTTP — cabe numa resposta de socket normal); importar lê um arquivo escolhido, valida a forma no
  cliente e mostra o resumo (importadas/sobrescritas/puladas, com nome e motivo) devolvido pelo ack.
- **Escopo do export/import**: só as entradas do compêndio da sala (decisão confirmada) — mapas,
  tokens, handouts, notas e encontros salvos ficam de fora (outras dependências, ex.: imagem
  hospedada; e um projeto bem maior se algum dia virar "exportar a sala inteira").
...
- **Mapas**: o `MapsPanel` que era o dropdown do `MapSelector` — miniaturas, criar, renomear,
  duplicar, apagar, reordenar, ativar, ponto de chegada, notas — e, no topo, "← Ir para o ativo" /
  "Ativar este" quando o mapa visto não é o ativo. **Imagem, grid e calibração NÃO migraram**:
  continuam no modal "Configurar Mapa" (`MapConfigModal`), aberto pela engrenagem ao lado do nome do
  mapa (§9.28).
- **Atalhos**: **M** abre a gaveta na seção Mapas e **Shift+P** na seção Preparo; a mesma tecla na
  mesma seção fecha. O botão do mapa na barra e os itens Handouts/Acervo/Preparo do ⋯ levam aos
  mesmos lugares; a engrenagem ao lado do nome do mapa continua abrindo o modal "Configurar Mapa".
