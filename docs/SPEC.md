...
- **Descrições** (texto do livro, fora do git): `--with-descriptions` converte o HTML do Foundry para texto simples (parágrafos, `**negrito**`, listas, links viram só o rótulo, aprimoramentos de magia viram lista "+N PM: ...") e grava `descriptions.local.json` (`{ id: texto }`, coberto por `*.local.json` no `.gitignore`). O servidor preenche `description` a partir dele ao montar o compêndio, se o arquivo existir; sem ele, preview e ficha mostram "Ver livro, pág. X".
